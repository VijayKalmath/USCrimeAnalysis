...
```{R}
df <- read_csv("data/processed/FBI_UCR/FBI_UCR_TABLE1.csv",show_col_types = FALSE)
df %>%
  kable(escape = F) %>% 
...
```{R}
df <- read_csv("data/processed/FBI_UCR/FBI_UCR_TABLE2_Incidents.csv",show_col_types = FALSE)
df %>%
  kable(escape = F) %>% 
  kable_styling("striped", font_size = 9, full_width = F) %>% 
  scroll_box(width = "500px", height = "300px") 
...
```{R}
df1 <- read_csv("data/processed/FBI_UCR/FBI_UCR_TABLE2_Victims.csv",show_col_types = FALSE)
df1 %>%
  kable(escape = F) %>% 
  kable_styling("striped", font_size = 9, full_width = F) %>% 
  scroll_box(width = "500px", height = "300px") 
...
### Hate Crime by State
```{R}
df1 <- read_csv("data/processed/FBI_UCR/FBI_UCR_TABLE11.csv",show_col_types = FALSE)
df1 %>%
  kable(escape = F) %>% 
  kable_styling("striped", font_size = 9, full_width = F) %>% 
  scroll_box(width = "500px", height = "300px") 
